locals {
  origin_id = "productA-CWAgent"
}

resource "aws_cloudfront_distribution" "cwagent_cloudfront" {

  comment = "ProductAアカウントのCWAgentアプリケーション用CloudFront" // CloudFrontの説明

  origin {
    /** CloudFrontに紐づけるDNS */
    domain_name = aws_alb.jvm_server.dns_name
  ...
  web_acl_id      = aws_wafv2_web_acl.cwagent_waf.arn // 紐付けるWAF ACL
...
resource "aws_cloudfront_cache_policy" "cloudfront_cache_policy" {
  name    = "ProductA-CWAgent-CloudfrontCachePolicy"
...
resource "aws_cloudfront_origin_request_policy" "cloudfront_origin_request_policy" {
  name    = "ProductA-CWAgent-CloudfrontOriginRequestPolicy"
