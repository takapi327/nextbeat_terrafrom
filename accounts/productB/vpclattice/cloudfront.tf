...
resource aws_cloudfront_distribution product_b_cloudfront {

  comment = "ProductBアカウント用のCloudFront" // CloudFrontの説明

  origin {
    /** CloudFrontに紐づけるDNS */
    domain_name = aws_alb.product_b_alb.dns_name
  ...
  web_acl_id      = aws_wafv2_web_acl.product_b_waf.arn // 紐付けるWAF ACL
...
resource aws_cloudfront_cache_policy cloudfront_cache_policy {
  name    = "ProductB-CloudfrontCachePolicy"
...
resource aws_cloudfront_origin_request_policy cloudfront_origin_request_policy {
  name    = "ProductB-CloudfrontOriginRequestPolicy"
